...
@Component({
    selector: 'luthier-manager-patches-lup-project',
    templateUrl: './luthier-manager-patches-lup-project.component.html',
    styleUrls: ['/luthier-manager-patches-lup-project.component.scss'],
...
export class LuthierManagerPatchesLupProjectComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges{
    ...
    public datasource = new MatTableDataSource<LuthierProjectModel>();
    displayedColumns = ['select', 'statusRow', 'name', 'version', 'ident', 'protocol'];
    selection = new SelectionModel<LuthierProjectModel>(true, []);
    ...
    ngAfterViewInit(): void {
        ...
        this.datasource.filterPredicate = (data: LuthierProjectModel, filter: string) => {
    ...
    toggleSelection(row: LuthierProjectModel) {
    ...
    handleFilter(filterData: FilterModel[]): void {
        ...
        if (!this.import) {
            ...
            this.service.filterProjects(filterRequest)
                .then(value => {
                    this.datasource.data = value;
                    this.applyFilter();
                });
