...
@Component({
    selector: 'luthier-manager-patches-lup-menu',
    templateUrl: './luthier-manager-patches-lup-menu.component.html',
    styleUrls: ['/luthier-manager-patches-lup-menu.component.scss'],
...
export class LuthierManagerPatchesLupMenuComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges{
    ...
    displayedColumns = ['select', 'statusRow', 'code', 'description', 'status', 'plataform'];
    selection = new SelectionModel<LuthierSubsystemModel>(true, []);
    ...
    filters: FilterModel[] = [
        ...
        },
        {
            ...
            operator: 'IN',
            required: false,
            options: Object.keys(LuthierSubsystemStatusEnum).map((key,index) => ({
                label: key,
                value: index
            }))
        },
        {
            ...
            operator: 'IN',
            required: false,
            options: Object.keys(LuthierPlataformEnum).map((key,index) => ({
                label: key,
                value: index
            }))
        }
    ...
    ngAfterViewInit(): void {
        ...
        this.datasource.filterPredicate = (data: LuthierSubsystemModel, filter: string) => {
    ...
    toggleSelection(row: LuthierSubsystemModel) {
    ...
    handleFilter(filterData: FilterModel[]): void {
        ...
        if (!this.import) {
            ...
            this.service.filterSubsystems(filterRequest)
                .then(value => {
