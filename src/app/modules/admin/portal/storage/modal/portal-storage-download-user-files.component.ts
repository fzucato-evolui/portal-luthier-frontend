...
export interface StorageDownloadUserFilesModalData {
    userId: number;
    userName: string;
}

@Component({
    selector: 'portal-storage-download-user-files',
    styleUrls: ['./portal-storage-download-user-files.component.scss'],
    templateUrl: './portal-storage-download-user-files.component.html',
    imports: [
        MatIconModule,
        MatButtonModule,
        MatDialogModule,
...
export class PortalStorageDownloadUserFilesComponent implements OnInit, OnDestroy {

    data: StorageDownloadUserFilesModalData;
    title: string = '';
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<PortalStorageDownloadUserFilesComponent>
    ) {}

    ngOnInit(): void {
        this.title = `Download dos arquivos do usuário: ${this.data.userName}`;

        this.abortFn = this._storageService.downloadUserFilesAsync(
            this.data.userId,
